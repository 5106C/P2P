...
/**
 * Host is responsible for parameters maintaining, establish connection to
 * running severs, and listen to connection request from other peers
 */
public class Host extends Thread {
	...
	/** constructor */
	public Host(int index, int peerID, int portNum, int fileStatus, int nPeers, HashMap<Integer, String[]> peerInfo,
			Common common) {
		...
		if (common.FileSize % common.PieceSize != 0)
			nPieces++;
		...
		String filePath = System.getProperty("user.dir") + File.separator + "peer_" + hostID + File.separator;
		...
		fp = new FileProcessing.FileProcess(common.FileName, filePath, filePath, common.FileSize, common.PieceSize);
	...
	private BitSet initBitfield(int fileStatus) {
		...
		if (fileStatus == 1)
			bitfield.flip(0, nPieces); // flip all bits if have complete file
	...
	public void run() {
		...
		// FileProcessing.FileProcess fp = new
		// FileProcessing.FileProcess(common.FileName, filePath,
		...
		synchronized (neighborsInfo) {
			for (Neighbor neighbor : neighborsInfo.values())
				neighbor.closeConnection();
		}
	...
	private void connectTo(int index, String[] peerInfo) {
		...
		PeerToPeer thread = new PeerToPeer(neighborsInfo, index, this.index, hostID, this.bitfield, pieceRequested,
				completedLabel, isInterestedOnMe, true, common, sharingRate, interestPeers);
		neighbor.setThread(thread);
		thread.start();
	}
